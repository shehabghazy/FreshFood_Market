...
async function accessAPI() {
    const getProductsFromAPI = await fetch(`http://localhost:${port}/products`)
    foodList = await getProductsFromAPI.json()
...
function displaySearchResult(getFood) {
    ...
    selectedFoodPrice = selectedFood.foodPrice.toFixed(2)
...
function displaycutomerCart() {
    ...
    for (const [getIndex , Food] of cutomerCart.entries()) {  
        ...
        <tr>
            ...
            <td>${Food.foodType}</td>
            <td class="text-center">${Food.foodPrice.toFixed(2)} $</td>
...
function displayDefault (){
    ...
    idSearchInput.value = ""
    openUpdateSectionError.innerHTML = ""
...
function displayFoodList() {
    ...
    for (let i = 0; i < foodList.length; i++) {
        ...
        <tr>
            ...
            <td colspan="2">${foodList[i].foodPrice.toFixed(2)} $</td>
        </tr>`        
    }
...
function addToFoodList(){
    ...
    for (const food of foodList) {
        if (createNewItem.itemId != '' && createNewItem.itemId == food.itemId) {
            isAvailable = true;
...
function saveInDatabase() {
    const sendToAPI = fetch(`http://localhost:${port}/products` , {
        method: "POST",
        body: JSON.stringify({
            newItemId: createNewItem.itemId,
            newFoodType: createNewItem.foodType,
            newFoodPhotoSrc: createNewItem.foodPhotoSrc,
            newFoodPrice: createNewItem.foodPrice
        }), 
        headers: {
...
function catchItemToUpdate() {
    ...
    updateFoodPrice.value = selectedFood.foodPrice.toFixed(2)
...
function updateInFoodList() {
    ...
    if (selectedFood.foodType == '' || selectedFood.foodPrice =='' || selectedFood.foodPhotoSrc == undefined) {
        displayUpdateSectionMsg('Please Fill All Inputs');
    }
    else {
...
function displayUpdateSectionMsg(msg) {
    switch (msg) {
        case 'Please Fill All Inputs':
            UpdateSectionMsg.classList.add('alert-warning')
            UpdateSectionMsg.classList.remove('alert-success')
            UpdateSectionMsg.innerHTML = `${msg} <i class="fa-solid fa-circle-exclamation"></i>`
            break;
        case 'Done':
...
function updateInDatabase() {
    const sendToAPI = fetch(`http://localhost:${port}/products` , {
        method: "PUT",
        body: JSON.stringify({
            updatedItemId: selectedFood.itemId,
            updatedFoodType: selectedFood.foodType,
            updatedFoodPhotoSrc: selectedFood.foodPhotoSrc,
            updatedFoodPrice: selectedFood.foodPrice
        }), 
        headers: {
            "Content-type": "application/json; charset=UTF-8"
        }
...
function deleteFromDatabase() {
    const sendToAPI = fetch(`http://localhost:${port}/products` , {
        method: "DELETE",
        body: JSON.stringify({
            deletedItemId: selectedFood.itemId,
        }), 
        headers: {
            "Content-type": "application/json; charset=UTF-8"
        }
...
function generateFatura(){
    ...
    // console.log(currentFaturaId);
    // console.log(JSON.stringify( currentFatura));
}
...
function displayFatura(){
    ...
    for (let i = 1; i < currentFatura.length; i++) {
        ...
        <span class="col-7 text-start">${currentFatura[i].foodType} * ${currentFatura[i].foodPrice.toFixed(2)} $</span>
        <span class="col-3 text-center">${currentFatura[i].QuantityPrice.toFixed(2)} $</span>
        </div>` 
    }
    getFaturaForDisplay += `
        <div class="row">
